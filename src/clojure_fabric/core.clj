(ns clojure-fabric.core
  "Hyperledger Java SDK functions"
...
(defn- args-from
  "Parameters are either a map, or seq of keys and values."
  [arg-seq]
  (let [args (first arg-seq)
        1st-arg (first args)]
    (if (map? 1st-arg)
      {:args (let [m (mapcat identity 1st-arg)]
               (if (empty? m)
                 {}
                 m))}
      {:args args})))
...
(defn intern-function
  ...
  (let [arglist (sort (map method-arglist methods))]
...
(defn generate-static-function
  "Generate a wrapper function defintion"
  [client fname methods]
  (let [defn (symbol "defn")
        class-prefix (str (.getName client) "/" )]
   (with-out-str
     (clojure.pprint/pprint
      `(~defn ~(symbol (name fname))
        ~@(map (fn [method args]
                 `([~@args]
                   (~(symbol (str class-prefix (.getName method))) ~@args)))
               methods (map method-arglist methods)))))))
...
(defn generate-function
  ...
  (let [defn (symbol "defn")
        ...
        type-hint (symbol (str "^" (.getName client)))]
    (with-out-str
      (clojure.pprint/pprint
       `(~defn ~(symbol (name fname))
         ~@(map (fn [method args]
                  `([~type-hint ~instance ~@args]
                    (. ~instance ~(symbol (.getName method)) ~@args)))
                methods (map method-arglist methods)))))))
...
(defn generate-client
  "Generate Clojure functions into the specified namespace all public methods
   from the fabric-sdk-java class as Clojure functions."
  [client]
  (concat
   ;; static functions
   (for [[fname methods] (client-methods client true)
         :let [fname2 (-> methods first .getName camel->keyword2)]]
     (generate-static-function client
                               (if (= fname fname2)
                                 fname
                                 fname2)
                               methods))
   ;; non-static functions
   (for [[fname methods] (client-methods client)
         :let [fname2 (-> methods first .getName camel->keyword2)]]
...
(def client (HFClient/createNewInstance))
